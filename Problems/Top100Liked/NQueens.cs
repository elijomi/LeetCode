namespace LeetCodeProblems.Top100Liked.NQueens;
...
public static class Runner
{
    public static void Run()
    {
        ...
        // Test cases
        var tests = new (int n, IList<IList<string>> expected)[]
        {
            (1, new List<IList<string>>
            {
                new List<string> { "Q" }
            }),
            (4, new List<IList<string>>
            {
                new List<string> { ".Q..","...Q","Q...","..Q." },
                new List<string> { "..Q.","Q...","...Q",".Q.." }
            }),
            (2, new List<IList<string>>()),
            (3, new List<IList<string>>())
        };
        ...
        foreach (var (n, expected) in tests)
        {
            ...
            bool pass = UnorderedBoardSetEqual(actual, expected);
            Console.WriteLine($"Test {i} (n={n}): expectedCount={expected.Count}, actualCount={actual.Count} - {(pass ? "PASS" : "FAIL")}");
            if (!pass)
            {
                Console.WriteLine($"  expected=[{FormatBoards(expected)}]");
                Console.WriteLine($"  actual  =[{FormatBoards(actual)}]");
            }
            i++;
        }
    }

    private static string FormatBoards(IList<IList<string>> boards)
        => string.Join(" ; ", boards.Select(b => $"[{string.Join(",", b)}]"));

    private static bool UnorderedBoardSetEqual(IList<IList<string>>? a, IList<IList<string>>? b)
    {
        if (ReferenceEquals(a, b)) return true;
        if (a is null || b is null) return false;

        var setA = new HashSet<string>(a.Select(BoardKey), StringComparer.Ordinal);
        var setB = new HashSet<string>(b.Select(BoardKey), StringComparer.Ordinal);
        return setA.SetEquals(setB);
    }

    private static string BoardKey(IList<string> board) => string.Join("|", board);
}
